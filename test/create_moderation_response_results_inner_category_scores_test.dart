//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openai/pi.dart';
import 'package:test/test.dart';

// tests for CreateModerationResponseResultsInnerCategoryScores
void main() {
  // final instance = CreateModerationResponseResultsInnerCategoryScores();

  group('test CreateModerationResponseResultsInnerCategoryScores', () {
    // num hate
    test('to test the property `hate`', () async {
      // TODO
    });

    // num hateSlashThreatening
    test('to test the property `hateSlashThreatening`', () async {
      // TODO
    });

    // num selfHarm
    test('to test the property `selfHarm`', () async {
      // TODO
    });

    // num sexual
    test('to test the property `sexual`', () async {
      // TODO
    });

    // num sexualSlashMinors
    test('to test the property `sexualSlashMinors`', () async {
      // TODO
    });

    // num violence
    test('to test the property `violence`', () async {
      // TODO
    });

    // num violenceSlashGraphic
    test('to test the property `violenceSlashGraphic`', () async {
      // TODO
    });
  });
}
