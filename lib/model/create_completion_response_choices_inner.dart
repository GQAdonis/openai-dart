...
class CreateCompletionResponseChoicesInner {
  ...
  @override
  String toString() => 'CreateCompletionResponseChoicesInner[text=$text, index=$index, logprobs=$logprobs, finishReason=$finishReason]';
  ...
  static Map<String, List<CreateCompletionResponseChoicesInner>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CreateCompletionResponseChoicesInner.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
